...
import org.bukkit.scheduler.BukkitTask; // Для PaydayTask
...
// Слушатели (убедись, что все используемые есть)
import org.isyateq.hfactions.listeners.*; // Импортируем все из пакета

// Менеджеры (убедись, что все используемые есть)
import org.isyateq.hfactions.managers.*; // Импортируем все из пакета

// Модели (обычно не нужны в главном классе)

// Задачи
import org.isyateq.hfactions.tasks.CuffLeashTask; // Если используется напрямую
import org.isyateq.hfactions.tasks.PaydayTask;

// Утилиты
import org.isyateq.hfactions.util.Utils; // Если используется напрямую
